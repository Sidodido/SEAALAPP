...
class App extends React.Component {
  state = {
    ...
    startLon: 3.438, // Default starting longitude
  ...
  _setRoutes = async () => {
    ...
    for (const item of data) {
      const destCoords = await this._getDestinationCoordinates(
        item.destination,
      );
      if (destCoords) {
        ...
        console.log(
          `Distance to ${item.destination}: ${distance.toFixed(2)} km`,
        );

        // Ajouter la destination et la distance au tableau
        destinationsWithDistances.push({
